...
function MapController({ followUser, location, setMapInstance }: { followUser: boolean, location: { lat: number, lng: number } | null, setMapInstance: (map: L.Map) => void }) {
  ...
  const lastCenteredRef = useRef<L.LatLng | null>(null);
  ...
  useEffect(() => {
    if (followUser && location) {
      ...
      const movedMeters = lastCenteredRef.current ? lastCenteredRef.current.distanceTo(target) : Infinity;
      if (movedMeters > 2) {
        // Keep camera pinned to live GPS without animation lag.
        map.setView([location.lat, location.lng], map.getZoom(), { animate: false });
        lastCenteredRef.current = target;
      }
    }
...
export function GameMap({ onMapLoad }: GameMapProps) {
  ...
  const breadcrumbTrail = useMemo(() => {
    ...
    const limit = followUser ? 180 : 260;
    return breadcrumbs.slice(-limit);
  }, [breadcrumbs, followUser]);
  ...
  useEffect(() => {
    ...
    // Only rotate map while actually moving to avoid jitter when stationary.
    if ((location.speed || 0) > 2.5 && Number.isFinite(location.heading)) {
      setMapRotation(location.heading || 0);
    }
  }, [followUser, location?.heading, location?.speed]);

  const rotation = followUser ? mapRotation : 0;
  ...
  return (
    <div className="w-full h-full absolute inset-0 z-0 bg-[#0f172a] overflow-hidden">
      ...
      {isOffRoute && (
        <div className="absolute top-[max(env(safe-area-inset-top),4.5rem)] left-1/2 -translate-x-1/2 z-50 pointer-events-auto px-2 w-full flex justify-center">
          <div className="w-full max-w-[34rem] bg-red-500/90 backdrop-blur-md px-4 py-3 md:px-6 md:py-4 rounded-2xl md:rounded-3xl border-2 border-white/20 shadow-[0_0_40px_rgba(239,68,68,0.5)] flex flex-col gap-3 md:gap-4 animate-in fade-in zoom-in duration-300">
      ...
      {cameraSettings.showFeed && (
        <div className="absolute bottom-[calc(env(safe-area-inset-bottom)+5.5rem)] right-3 w-40 h-24 md:bottom-6 md:right-6 md:w-64 md:h-36 bg-black rounded-2xl border border-white/20 shadow-2xl overflow-hidden z-50 group">
      ...
      >
        ...
        >
          ...
          {isRecording && breadcrumbTrail.length > 1 && (
            <Polyline 
              positions={breadcrumbTrail.map(p => [p.lat, p.lng])}
              pathOptions={{
                color: mapStyle === 'game-night' ? '#a78bfa' : '#8b5cf6', // Violet-400 or Violet-500
                weight: 2,
                opacity: 0.5,
                lineJoin: 'round',
                dashArray: '1, 12' // Dot-like trail
              }}
            />
          )}
