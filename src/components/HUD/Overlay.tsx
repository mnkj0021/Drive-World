...
import { Gauge, Navigation, Users, Radio, Play, Square, Map as MapIcon, Zap, Sun, Moon, Crosshair, Share2, X, Sparkles, Camera, Eye, EyeOff, Settings, Trophy, Signal, Menu } from 'lucide-react';
...
export function HUD() {
  ...
  const [isMobileQuickActionsOpen, setIsMobileQuickActionsOpen] = React.useState(false);
  ...
  return (
    <>
      ...
      <div className="fixed right-3 md:right-6 bottom-[calc(env(safe-area-inset-bottom)+0.75rem)] md:bottom-auto md:top-[max(env(safe-area-inset-top),0.5rem)] z-[120] pointer-events-auto flex flex-col gap-2">
        {/* GPS Signal Indicator */}
        {location && location.accuracy && (
          <div className={cn(
            "hidden md:flex w-10 h-10 md:w-12 md:h-12 rounded-full items-center justify-center border border-white/10 backdrop-blur-xl shadow-lg transition-all",
        ...
        <button 
          ...
          className="hidden md:flex w-10 h-10 md:w-12 md:h-12 rounded-full items-center justify-center border border-white/10 bg-black/60 backdrop-blur-xl text-gray-400 hover:bg-black/80 hover:text-white transition-all shadow-lg"
          title="Settings"
        >
          <Settings size={18} />
        </button>
      ...
      <div className={cn(
        ...
        <div className="flex flex-col md:flex-row items-center justify-between pointer-events-auto w-full gap-2 px-3 md:px-6 pt-[max(env(safe-area-inset-top),0.75rem)] pb-2 md:py-6 z-50">
          ...
          <div className="w-full md:w-auto flex justify-between items-center md:contents order-2 md:order-1">
            ...
            {/* Right: Controls */}
            <div className="relative flex gap-1.5 md:gap-2 flex-shrink-0 items-center md:order-3 overflow-x-auto md:overflow-visible max-w-[calc(100vw-1rem)] md:max-w-none pr-1">
              ...
              <button 
                ...
                className="hidden md:flex w-10 h-10 md:w-auto md:h-auto md:px-4 md:py-3 rounded-full md:rounded-2xl items-center justify-center border border-white/10 bg-black/60 backdrop-blur-xl text-gray-400 hover:bg-black/80 hover:text-white transition-all shadow-lg"
                title="Run History"
              >
                <MapIcon size={18} />
              ...
              <button 
                ...
                className={cn(
                  "hidden md:flex w-10 h-10 md:w-auto md:h-auto md:px-4 md:py-3 rounded-full md:rounded-2xl items-center justify-center border transition-all backdrop-blur-xl shadow-lg",
                  isAssistantOpen 
                    ? "bg-emerald-500/20 border-emerald-500 text-emerald-400 shadow-[0_0_20px_rgba(16,185,129,0.3)]" 
                    : "bg-black/60 border-white/10 text-gray-400 hover:bg-black/80 hover:text-white"
                )}
              ...
              {/* Mobile Settings */}
              <button
                onClick={() => setIsMobileQuickActionsOpen((prev) => !prev)}
                className={cn(
                  "md:hidden h-10 rounded-full flex items-center justify-center gap-1.5 border backdrop-blur-xl transition-all shadow-lg px-3",
                  isMobileQuickActionsOpen
                    ? "bg-emerald-500/20 border-emerald-500 text-emerald-400"
                    : "border-white/10 bg-black/60 text-gray-400 hover:bg-black/80 hover:text-white"
                )}
                title="More Actions"
              >
                {isMobileQuickActionsOpen ? <X size={18} /> : <Menu size={18} />}
                <span className="text-[11px] font-bold uppercase tracking-wider">More</span>
              </button>

              <button 
                onClick={toggleSettings}
                className="md:hidden w-10 h-10 rounded-full flex items-center justify-center border border-white/10 bg-black/60 backdrop-blur-xl text-gray-400 hover:bg-black/80 hover:text-white transition-all shadow-lg"
                title="Settings"
              >
                <Settings size={18} />
              </button>
              ...
              {isMobileQuickActionsOpen && (
                <div className="md:hidden absolute top-full right-0 mt-2 w-48 bg-black/90 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-2 z-[120]">
                  <button
                    onClick={() => {
                    ...
                    className={cn(
                      "w-full flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-bold transition-colors",
                      isAssistantOpen
                  ...
                  >
                    <Sparkles size={15} />
                    {isAssistantOpen ? "Assistant On" : "Assistant Off"}
                  </button>
                  ...
                  <button
                    ...
                    className="w-full flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-bold text-gray-200 hover:bg-white/10 transition-colors"
                  >
                    <MapIcon size={15} />
                    History
                  </button>
                  ...
                  >
                    <Zap size={15} />
                    {isSimulatorActive ? "Simulator On" : "Simulator Off"}
                  </button>

                  <div className="mt-1 px-1 py-1.5 border-t border-white/10 flex items-center justify-between">
                    <span className="text-[11px] font-semibold text-gray-300">Legend</span>
                    <MapLegend />
                  </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 flex items-end justify-between px-3 md:px-4 pb-[calc(env(safe-area-inset-bottom)+0.5rem)] md:pb-8 pointer-events-none">
          
          {/* Left: Controls */}
          <div className="flex flex-row md:flex-col gap-2 md:gap-3 pointer-events-auto mb-0 bg-black/60 md:bg-transparent backdrop-blur-xl md:backdrop-blur-0 border border-white/10 md:border-0 rounded-2xl md:rounded-none px-2.5 py-2 md:p-0">
             {/* Run Control */}
             <div className="flex flex-col gap-0.5 md:gap-1 items-center">
               <button
                onClick={() => setRecording(!isRecording)}
                className={cn(
                  "w-11 h-11 md:w-16 md:h-16 rounded-full flex items-center justify-center border-2 transition-all shadow-lg hover:scale-105 active:scale-95",
                  isRecording 
              ...
              <div className="hidden md:block text-center font-mono text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em]">
            ...
            <div className="flex flex-col gap-0.5 md:gap-1 items-center">
              ...
              <div className="hidden md:block text-center font-mono text-[8px] font-bold text-blue-500/60 uppercase tracking-widest">
                FOLLOW
              </div>
            </div>
            ...
            <div className="flex flex-col gap-0.5 md:gap-1 items-center">
              ...
              <div className="hidden md:block text-center font-mono text-[8px] font-bold text-emerald-500/60 uppercase tracking-widest">
                CENTER
              </div>
            </div>
          </div>
          ...
          {/* Center: Speedometer */}
          <div className="absolute left-1/2 -translate-x-1/2 bottom-[6.35rem] md:bottom-8 pointer-events-none z-0 scale-[0.34] sm:scale-[0.42] md:scale-[0.8] origin-bottom">
          ...
          {/* Right: Stats */}
          <div className={cn(
            "pointer-events-auto absolute right-3 bottom-[6.2rem] md:static backdrop-blur-xl border border-white/10 rounded-2xl p-2 md:p-4 mb-0 md:mb-10 shadow-lg transition-all text-right min-w-[88px] md:min-w-0",
            mapStyle === 'game-day' ? "bg-white/80 border-black/10" : "bg-black/60 border-white/20"
          )}>
            <div className="mb-2">
              <div className="text-[8px] md:text-[10px] text-gray-500 font-bold uppercase tracking-wider mb-0.5 md:mb-1">Distance</div>
              <div className={cn("text-sm sm:text-base md:text-2xl font-mono font-bold", mapStyle === 'game-day' ? "text-slate-800" : "text-white")}>
                {distance.toFixed(1)} <span className="text-[9px] md:text-sm text-gray-500 font-normal">KM</span>
              </div>
            </div>
            <div>
              <div className="text-[8px] md:text-[10px] text-gray-500 font-bold uppercase tracking-wider mb-0.5 md:mb-1">Time</div>
              <div className={cn("text-sm sm:text-base md:text-2xl font-mono font-bold", mapStyle === 'game-day' ? "text-slate-800" : "text-white")}>
